window.__ModuleLoader__.load({
  id: "@cmx666/dsh-client-ui-niubash",
  factory: (require) => {
    const module = { exports: {} };
    const React = require("react");
    const h = React.createElement;
    const NS = "settings.niubash";
    ...
    const copy = {
      en: {
        ...
        requiresPath: "Requires the niu (Niubash) executable on PATH.",
      ...
      zh: {
        ...
        restart: "重启 DSH 后生效。",
        requiresPath: "需要 PATH 中存在 niu（Niubash）可执行文件。",
        exportSession: "导出会话",
      },
    };
    ...
    function NiubashCard() {
      const lang = language();
      const t = copy[lang];
      const [enabled, setEnabled] = React.useState(() => {
        try { return localStorage.getItem("dsh.niubash.enabled") !== "0"; } catch { return true; }
      });
      const update = (event) => {
        const next = event.currentTarget.checked;
        setEnabled(next);
        try { localStorage.setItem("dsh.niubash.enabled", next ? "1" : "0"); } catch {}
      };
      return h("article", {
        style: {
    ...
    function apply(ctx) {
      ctx.effect(() => ctx.locale.register(NS, copy), "ui-niubash: dictionaries");
      ...
      ctx.slots.inject("conversation.session.header.actions", () => ctx.slots.register({
        ...
        id: "niubash-session-log-download-action",
        order: 100,
        locale: NS,
        inject: () => ({
