 ...
 * `spawnSpec()` — so this subclass maps `niubashPath` onto the base's
 ...
 * niubash-branded messages; the base re-validates with its own prefix.
 *
 ...
 *
 * @module @cmx666/dsh-niubash-local
 */
 ...
 */
function candidateNiubashPaths(env = process.env, platform = process.platform) {
 ...
 * @param {string | undefined} configured - an explicit `niubashPath` config value, trusted as-is.
 * @param {NodeJS.ProcessEnv} [env] - the environment to probe; defaults to the process environment.
 * @param {NodeJS.Platform} [platform] - the platform to resolve for; defaults to the process platform.
 ...
 */
function resolveNiubashPath(configured, env = process.env, platform = process.platform) {
 ...
 * failing at the next command. Niubash-branded: the base executor would
 * report the same facts under its own `pwsh-local` prefix.
 */
function assertNiubashConfig(config) {
  for (const name of ['timeoutMs', 'maxTimeoutMs', 'maxOutputBytes', 'maxSpillBytes', 'graceMs']) {
    const value = config[name]
    if (!Number.isFinite(value) || value <= 0) throw new Error(`niubash-local: ${name} must be a positive finite number`)
  }
  if (config.graceMs > MAX_TIMER_DELAY_MS) throw new Error(`niubash-local: graceMs must be no greater than ${MAX_TIMER_DELAY_MS}`)
}
 ...
 * bash-like model-friendly environment, and niubash-branded diagnostics.
 */
class NiubashLocalExecutor extends PwshLocalExecutor {
  static Config = Schema.object({
    ...
    niubashPath: Schema.string(),
  })

  constructor(ctx, config) {
    const declared = config?.niubashPath
    const pwshPath = resolveNiubashPath(declared)
    if (config !== undefined) assertNiubashConfig(config)
    super(ctx, { ...config, pwshPath })
    /** The declared niubashPath the current resolved executable came from. */
    this.declaredNiubashPath = declared
    /** The winuxsh executable resolved from the current config. */
    this.resolvedNiubashPath = pwshPath
  }

  /**
   * The winuxsh executable every command runs through. Re-resolves when a
   * stored `niubashPath` differs from the one the current executable was
   * resolved from, so an unrelated settings change never re-probes the
   * filesystem. (schemastery preserves the unknown-to-the-base `niubashPath`
   * key through the settings layer, which is what makes this readable.)
   */
  get niubashPath() {
    const declared = this.config?.niubashPath
    if (declared !== this.declaredNiubashPath) {
      this.declaredNiubashPath = declared
      this.resolvedNiubashPath = resolveNiubashPath(declared)
    }
    return this.resolvedNiubashPath
  }

  /**
   * Resolve a request into a fully-specified spec, with niubash-branded
  ...
  resolve(request) {
    if (request.timeoutMs !== undefined && (!Number.isFinite(request.timeoutMs) || request.timeoutMs <= 0)) {
      throw new Error('niubash-local: request.timeoutMs must be a positive finite number')
    }
    if (request.stdoutMaxBytes !== undefined && (!Number.isFinite(request.stdoutMaxBytes) || request.stdoutMaxBytes <= 0)) {
      throw new Error('niubash-local: request.stdoutMaxBytes must be a positive finite number')
    }
   ...
   * `@cmx666/dsh-niubash-sandbox`).
   */
  argv(spec) {
    return [this.niubashPath, '-c', spec.command]
  }
   ...
   * concept, so niubash — unlike pwsh — honors it).
   */
...
export { NiubashLocalExecutor, assertNiubashConfig, candidateNiubashPaths, resolveNiubashPath }
export default NiubashLocalExecutor
