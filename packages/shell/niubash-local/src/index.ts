...
import { resolveNiubashPath } from './resolve.ts'
...
/** The shape after schemastery applied the defaults (cwd/niubashPath have none). */
export type ResolvedNiubashConfig = Required<Omit<Config, 'cwd' | 'niubashPath'>> & Pick<Config, 'cwd' | 'niubashPath'>
/* jscpd:ignore-end */
...
export { candidateNiubashPaths, resolveNiubashPath } from './resolve.ts'
 ...
 */
export class NiubashLocalExecutor extends PwshLocalExecutor {
  ...
  /** Niubash-branded diagnostics from the inherited executor mechanics. */
  protected override readonly diagnosticPrefix = 'niubash-local'

  /** Reject the resolved section with niubash-branded messages. */
  protected override assertServiceable(config: PwshConfig): void {
    ...
    assertServiceablePwshConfig(config, 'niubash-local')
  }
  ...
  protected override declaredExecutable(config: ResolvedConfig): string | undefined {
    return (config as unknown as ResolvedNiubashConfig).niubashPath
  }
  ...
  protected override resolveExecutable(declared: string | undefined): string {
    return resolveNiubashPath(declared)
  }

  /** The winuxsh executable every command runs through. */
  get niubashPath(): string {
   ...
   * of `dsh-pwsh-local`'s argv hook; see `@cmx666/dsh-niubash-sandbox`).
   */
