/**
 * Real-process tests for `@cmx666/dsh-niubash-local`: the LOCAL subprocess
...
import { NiubashLocalExecutor, candidateNiubashPaths, resolveNiubashPath } from '@cmx666/dsh-niubash-local'
...
const hasNiubash = spawnSync(resolveNiubashPath(), ['-c', 'exit 0'], { encoding: 'utf8' }).status === 0

/** Normalize platform line endings (CRLF on Windows, LF elsewhere). */
const lf = (text: string): string => text.replace(/\r\n/g, '\n')

async function setup(config: ConstructorParameters<typeof NiubashLocalExecutor>[1] = {}) {
  const ctx = new Context()
  await ctx.plugin(LocalSubprocessRuntime)
  await ctx.plugin(NiubashLocalExecutor, { graceMs: 200, ...config })
  const shell = ctx.shell as NiubashLocalExecutor
  return { ctx, shell }
}

describe('resolveNiubashPath and candidateNiubashPaths (pure, every platform)', () => {
  it('trusts an explicit configured path verbatim', () => {
    expect(resolveNiubashPath('C:\\custom\\winuxsh.exe')).toBe('C:\\custom\\winuxsh.exe')
    expect(resolveNiubashPath('winuxsh')).toBe('winuxsh')
  })

  it('falls through an empty configured path to PATH discovery', () => {
    expect(resolveNiubashPath('', { PATH: 'P:\\Store' }, 'win32')).toBe('niu')
  })

  it('names the executable by platform, niu preferred globally', () => {
    expect(candidateNiubashPaths({ PATH: 'P:\\Store' }, 'win32'))
      .toEqual([join('P:\\Store', 'niu.exe'), join('P:\\Store', 'winuxsh.exe')])
    expect(candidateNiubashPaths({ PATH: '/usr/local/bin' }, 'linux'))
      .toEqual([join('/usr/local/bin', 'niu'), join('/usr/local/bin', 'winuxsh')])
    expect(candidateNiubashPaths({ PATH: '/usr/local/bin' }, 'darwin'))
      .toEqual([join('/usr/local/bin', 'niu'), join('/usr/local/bin', 'winuxsh')])
  })
  ...
  it('splits PATH by the platform delimiter and strips surrounding quotes', () => {
    ...
    expect(win32).toEqual([join('Q:\\quoted store', 'niu.exe'), join('Q:\\quoted store', 'winuxsh.exe')])
    expect(candidateNiubashPaths({}, 'win32')).toEqual([])
    const posix = candidateNiubashPaths({ PATH: '"/opt/store":/usr/bin:' }, 'linux')
    // niu candidates are collected globally before the legacy winuxsh ones.
    expect(posix).toEqual([
      join('/opt/store', 'niu'),
      join('/usr/bin', 'niu'),
      join('/opt/store', 'winuxsh'),
      join('/usr/bin', 'winuxsh'),
    ])
  })

  it('returns the first EXISTING PATH candidate, else niu', () => {
    const dir = mkdtempSync(join(tmpdir(), 'dsh-niubash-resolve-'))
    const store = join(dir, 'store')
    mkdirSync(store, { recursive: true })
    writeFileSync(join(store, 'winuxsh.exe'), '')
    expect(resolveNiubashPath(undefined, { PATH: store }, 'win32')).toBe(join(store, 'winuxsh.exe'))
    expect(resolveNiubashPath(undefined, { PATH: join(dir, 'empty') }, 'win32')).toBe('niu')
    expect(resolveNiubashPath(undefined, { PATH: store }, 'linux')).toBe('niu')
  })

  it('a later PATH entry niu.exe beats an earlier winuxsh.exe', () => {
    const dir = mkdtempSync(join(tmpdir(), 'dsh-niubash-resolve-order-'))
    const first = join(dir, 'first')
    const second = join(dir, 'second')
    mkdirSync(first, { recursive: true })
    mkdirSync(second, { recursive: true })
    writeFileSync(join(first, 'winuxsh.exe'), '')
    writeFileSync(join(second, 'niu.exe'), '')
    expect(resolveNiubashPath(undefined, { PATH: `${first};${second}` }, 'win32')).toBe(join(second, 'niu.exe'))
  })

  it('accepts a link-shaped PATH candidate whose target cannot be stat-ed', (ctx) => {
    const dir = mkdtempSync(join(tmpdir(), 'dsh-niubash-resolve-link-'))
    const store = join(dir, 'store')
    mkdirSync(store, { recursive: true })
    ...
    try {
      ...
      // Creating a (dangling) file symlink on Windows needs Developer Mode or
      // an elevated token; POSIX environments always have the privilege and
      // keep the real coverage.
      if ((error as NodeJS.ErrnoException).code === 'EPERM') ctx.skip()
      throw error
    }
    expect(resolveNiubashPath(undefined, { PATH: store }, 'win32')).toBe(link)
  })

  it('skips a directory candidate and falls through to the PATH-resolution default', () => {
    const dir = mkdtempSync(join(tmpdir(), 'dsh-niubash-resolve-dir-'))
    const store = join(dir, 'store')
    mkdirSync(join(store, 'winuxsh.exe'), { recursive: true })
    expect(resolveNiubashPath(undefined, { PATH: store }, 'win32')).toBe('niu')
  })
})

describe('config validation (pure, every platform)', () => {
  it('rejects invalid numeric config at construction', async () => {
    await expect(setup({ timeoutMs: Number.NaN })).rejects.toThrow(/niubash-local: timeoutMs/)
    await expect(setup({ maxTimeoutMs: 0 })).rejects.toThrow(/niubash-local: maxTimeoutMs/)
    await expect(setup({ maxOutputBytes: -1 })).rejects.toThrow(/niubash-local: maxOutputBytes/)
    await expect(setup({ maxSpillBytes: 0 })).rejects.toThrow(/niubash-local: maxSpillBytes/)
    await expect(setup({ graceMs: 0 })).rejects.toThrow(/niubash-local: graceMs/)
    await expect(setup({ graceMs: MAX_TIMER_DELAY_MS + 1 }))
      .rejects.toThrow(`niubash-local: graceMs must be no greater than ${MAX_TIMER_DELAY_MS}`)
  })

  it('rejects invalid per-call overrides in resolve()', async () => {
    const { ctx, shell } = await setup()
    expect(() => shell.resolve({ command: 'echo ok', timeoutMs: Number.NaN })).toThrow(/niubash-local: request\.timeoutMs/)
    expect(() => shell.resolve({ command: 'echo ok', timeoutMs: -1 })).toThrow(/niubash-local: request\.timeoutMs/)
    expect(() => shell.resolve({ command: 'echo ok', stdoutMaxBytes: Number.NaN })).toThrow(/niubash-local: request\.stdoutMaxBytes/)
    expect(() => shell.resolve({ command: 'echo ok', stdoutMaxBytes: -1 })).toThrow(/niubash-local: request\.stdoutMaxBytes/)
...
describe('spawn construction (pure, every platform)', () => {
  ...
  it('runs every command as ONE argv element under `-c`', async () => {
    ...
    await ctx.plugin(NiubashLocalExecutor)
    ...
    // path when niu/winuxsh is installed, else the bare `niu` fallback.
    expect(argv[0]).toBe(resolveNiubashPath())
    expect(argv[1]).toBe('-c')
    expect(argv[2]).toBe('echo 你好')
    await ctx.fiber.dispose()
  })

  it('uses the configured niubashPath verbatim and threads the bash-like terminal env', async () => {
    const ctx = new Context()
    const subprocess = new CapturingSubprocessRuntime(ctx)
    await ctx.plugin(NiubashLocalExecutor, { niubashPath: 'C:\\tools\\winuxsh.exe' })
...
describe.skipIf(!hasNiubash)('NiubashLocalExecutor.run', () => {
  ...
  it('uses config cwd, overridable per call', async () => {
    const first = mkdtempSync(join(tmpdir(), 'dsh-niubash-cwd-a-'))
    const second = mkdtempSync(join(tmpdir(), 'dsh-niubash-cwd-b-'))
...
describe.skipIf(!hasNiubash)('NiubashLocalExecutor.start (background process handles)', () => {
