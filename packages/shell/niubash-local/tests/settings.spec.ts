...
import { NiubashLocalExecutor } from '@cmx666/dsh-niubash-local'
...
async function boot(config: ConstructorParameters<typeof NiubashLocalExecutor>[1] = {}): Promise<{
  ctx: Context
  settingsFiber: Fiber
  executorFiber: Fiber
  winuxsh: NiubashLocalExecutor
}> {
  const ctx = new Context()
  await ctx.plugin(LocalSubprocessRuntime)
  const settingsFiber = ctx.plugin(MemorySettings)
  await settingsFiber.await()
  const executorFiber = ctx.plugin(NiubashLocalExecutor, { timeoutMs: 60_000, ...config })
  await executorFiber.await()
  return { ctx, settingsFiber, executorFiber, winuxsh: ctx.shell as NiubashLocalExecutor }
}
...
describe('winuxsh executor over the bash settings section', () => {
  ...
  it('refuses a stored value the constructor would have rejected', async () => {
    ...
    // The stored-value write gate is the pwsh base's registered validate hook
    // until upstream publishes the hook-based executor family; either prefix
    // reports the same refusal.
    await expect(bench.ctx.settings.update(SHELL_SETTINGS_NAMESPACE, { timeoutMs: 0 }))
      .rejects.toThrow(/(?:winuxsh|pwsh)-local: timeoutMs must be a positive finite number/)
  ...
  it('re-resolves the executable when the stored path changes', async () => {
    const bench = await boot({ niubashPath: '/opt/first/winuxsh' })
    expect(bench.winuxsh.niubashPath).toBe('/opt/first/winuxsh')

    await bench.ctx.settings.update(SHELL_SETTINGS_NAMESPACE, { niubashPath: '/opt/second/winuxsh' })

    expect(bench.winuxsh.niubashPath).toBe('/opt/second/winuxsh')
    await bench.ctx.fiber.dispose()
  })

  it('keeps the resolved executable when an unrelated field changes', async () => {
    const bench = await boot({ niubashPath: '/opt/first/winuxsh' })
    const before = bench.winuxsh.niubashPath

    await bench.ctx.settings.update(SHELL_SETTINGS_NAMESPACE, { timeoutMs: 5_000 })

    expect(bench.winuxsh.niubashPath).toBe(before)
    await bench.ctx.fiber.dispose()
  })

  it('falls back to the composition entry when the settings provider detaches', async () => {
    const bench = await boot({ niubashPath: '/opt/first/winuxsh' })
    await bench.ctx.settings.update(SHELL_SETTINGS_NAMESPACE, { timeoutMs: 5_000, niubashPath: '/opt/second/winuxsh' })
    expect(bench.winuxsh.config.timeoutMs).toBe(5_000)
    expect(bench.winuxsh.niubashPath).toBe('/opt/second/winuxsh')
