/**
 * Consumer-side `SandboxNiubashExecutor` tests. A fake Cordis sandbox service
 * makes wrapping, policy hand-off, fail-closed propagation, and fact stamping
 * deterministic; real-provider integration lives in the pwsh twin's
 * `tests/acl.e2e.ts` (the ACL runner is provider-side, not executor-side).
 * Requires winuxsh for the integration block (skips without it — same gate as
 * winuxsh-local's suites); the helpers block is pure and always runs.
 */
...
import { resolveNiubashPath } from '@cmx666/dsh-niubash-local'
import { SandboxPolicyService } from '@deepseek-ai/dsh-sandbox-policy'
import LocalSubprocessRuntime from '@deepseek-ai/dsh-subprocess-local'
import { SandboxNiubashExecutor, classifyRunnerFailure, isRunnerSpawnFailure, matchesSignature } from '@cmx666/dsh-niubash-sandbox'

// The same probe winuxsh-local's suites and the vitest coverage exemption use:
// spawnSync never throws on a missing binary (it reports status null) — only
// the status is truth.
function winuxshAvailable(): boolean {
  return spawnSync(resolveNiubashPath(), ['-c', 'exit 0'], { encoding: 'utf8' }).status === 0
}

const spillDir = mkdtempSync(join(tmpdir(), 'dsh-niubash-sandbox-spec-'))
...
async function setup(
...
): Promise<{ executor: SandboxNiubashExecutor; calls: ConfineCall[] }> {
  ...
  await ctx.plugin(SandboxNiubashExecutor, { graceMs: 200 })
  return { executor: ctx.shell as SandboxNiubashExecutor, calls }
}

describe('helpers (pure)', () => {
  const workdir = mkdtempSync(join(tmpdir(), 'dsh-niubash-sandbox-helpers-'))
  afterAll(() => {
...
describe.skipIf(!winuxshAvailable())('SandboxNiubashExecutor', () => {
  // Denial device for the POSIX classification cases: a mode-0555 directory
  // INSIDE a temp scratch tree (the same device as bash/pwsh-sandbox's
  // suites) — unit tests never attempt writes outside the system temp
  // directory. On win32 there is no POSIX mode denial; the real-sandbox denial
  // coverage lives in the pwsh twin's tests/acl.e2e.ts, where the ACL runner
  // denies scratch paths (the runner is provider-side, shared by winuxsh).
  const readOnlyDir = mkdtempSync(join(tmpdir(), 'dsh-niubash-sandbox-ro-'))
  if (process.platform !== 'win32') chmodSync(readOnlyDir, 0o555)
  const deniedWriteCommand = `printf 'x' > '${join(readOnlyDir, 'probe.txt')}'`
  ...
  it('wraps the exact winuxsh argv through ctx.sandbox with the per-call policy', async () => {
    ...
    // argv[0] is whatever resolveNiubashPath picked: the PATH niu[.exe] when
    // Niubash is installed, else the legacy winuxsh[.exe] or a bare niu.
    expect(call?.argv[0]).toMatch(/(?:^|[\\/])(?:niu|winuxsh)(?:\.exe)?$/u)
  ...
  it('an aborted caller signal outranks runner-spawn attribution', async () => {
    ...
    const { executor } = await setup(() => ({
      argv: ['definitely-not-a-real-runner', '--', 'winuxsh'],
      enforcement: 'full',
      denialSignatures: [],
      runnerFailureRules: [],
    }))
  ...
  // real-sandbox denial classification is covered by the pwsh twin's
  // tests/acl.e2e.ts (the ACL runner denies scratch paths — unit tests never
  // leave temp).
  ...
  // coverage lives in the pwsh twin's tests/acl.e2e.ts.
