/**
 * Consumer-side `SandboxWinuxshExecutor` tests. A fake Cordis sandbox service
...
import { SandboxWinuxshExecutor, classifyRunnerFailure, isRunnerSpawnFailure, matchesSignature } from '@cmx666/dsh-winuxsh-sandbox'
...
function winuxshAvailable(): boolean {
  return spawnSync(resolveWinuxshPath(), ['-c', 'exit 0'], { encoding: 'utf8' }).status === 0
}

const spillDir = mkdtempSync(join(tmpdir(), 'dsh-winuxsh-sandbox-spec-'))
...
async function setup(
...
): Promise<{ executor: SandboxWinuxshExecutor; calls: ConfineCall[] }> {
  ...
  await ctx.plugin(SandboxWinuxshExecutor, { graceMs: 200 })
  return { executor: ctx.shell as SandboxWinuxshExecutor, calls }
}

describe('helpers (pure)', () => {
  const workdir = mkdtempSync(join(tmpdir(), 'dsh-winuxsh-sandbox-helpers-'))
  afterAll(() => {
...
describe.skipIf(!winuxshAvailable())('SandboxWinuxshExecutor', () => {
  ...
  const readOnlyDir = mkdtempSync(join(tmpdir(), 'dsh-winuxsh-sandbox-ro-'))
  ...
  it('wraps the exact winuxsh argv through ctx.sandbox with the per-call policy', async () => {
    ...
    // argv[0] is whatever resolveWinuxshPath picked: the PATH niu[.exe] when
