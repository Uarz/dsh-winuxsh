/**
 * Real-process tests for `@cmx666/dsh-winuxsh-local`: the LOCAL subprocess
...
import { WinuxshLocalExecutor, candidateWinuxshPaths, resolveWinuxshPath } from '@cmx666/dsh-winuxsh-local'
...
describe('resolveWinuxshPath and candidateWinuxshPaths (pure, every platform)', () => {
  ...
  it('falls through an empty configured path to PATH discovery', () => {
    expect(resolveWinuxshPath('', { PATH: 'P:\\Store' }, 'win32')).toBe('niu')
  })

  it('names the executable by platform, niu preferred globally', () => {
    expect(candidateWinuxshPaths({ PATH: 'P:\\Store' }, 'win32'))
      .toEqual([join('P:\\Store', 'niu.exe'), join('P:\\Store', 'winuxsh.exe')])
    expect(candidateWinuxshPaths({ PATH: '/usr/local/bin' }, 'linux'))
      .toEqual([join('/usr/local/bin', 'niu'), join('/usr/local/bin', 'winuxsh')])
    expect(candidateWinuxshPaths({ PATH: '/usr/local/bin' }, 'darwin'))
      .toEqual([join('/usr/local/bin', 'niu'), join('/usr/local/bin', 'winuxsh')])
  })
  ...
  it('returns the first EXISTING PATH candidate, else niu', () => {
    ...
    expect(resolveWinuxshPath(undefined, { PATH: join(dir, 'empty') }, 'win32')).toBe('niu')
    expect(resolveWinuxshPath(undefined, { PATH: store }, 'linux')).toBe('niu')
  })

  it('a later PATH entry niu.exe beats an earlier winuxsh.exe', () => {
    const dir = mkdtempSync(join(tmpdir(), 'dsh-winuxsh-resolve-order-'))
    ...
    expect(resolveWinuxshPath(undefined, { PATH: `${first};${second}` }, 'win32')).toBe(join(second, 'niu.exe'))
  })
  ...
  it('skips a directory candidate and falls through to the PATH-resolution default', () => {
    ...
    expect(resolveWinuxshPath(undefined, { PATH: store }, 'win32')).toBe('niu')
  })
})
